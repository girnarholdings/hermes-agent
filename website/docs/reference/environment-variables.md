...
| `HERMES_CRON_TIMEOUT` | Backward-compatible override for `cron.agent_inactivity_timeout_seconds` (default: `600`). The agent can run indefinitely while actively calling tools or receiving stream tokens — this only triggers when idle. Set to `0` for unlimited. Prefer `config.yaml` for non-secret settings. |
