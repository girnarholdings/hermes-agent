...
def build_context_files_prompt(
    ...
    if (
        ...
        # install-tree fallback: no project context gains system-prompt authority
    else:
        # HERMES.md is the primary project context (machine operating manual).
        hermes_context = _load_hermes_md(cwd_path, context_length)

        # AGENTS.md is the auto-generated Layer-0 nav index — a DIFFERENT role
        # from HERMES.md, so it loads ALONGSIDE it (never shadowed). When no
        # HERMES.md is present, AGENTS.md/CLAUDE.md/.cursorrules remain
        # mutually-exclusive alternates (first match wins) since they are
        # ecosystem equivalents. AGENTS.md still passes through
        # _scan_context_content (inside _load_agents_md) — injection guard intact.
        if hermes_context:
